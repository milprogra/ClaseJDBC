...
public class AccesoProducto extends Conexion {
    ...
    public ResultSet listarProductoCategoria(){
        try {
            super.getStmt();
            resultado = stmt.executeQuery("SELECT  `producto`.`id`,`modelo`,`procesador`,`ram`,`disco`,`precio`,`nombre` FROM `producto` inner join `categoria` on `producto`.`categoria_id`=`categoria`.`id`");
        } catch (SQLException ex) {
            Logger.getLogger(AccesoProducto.class.getName()).log(Level.SEVERE, null, ex);
        }
        return resultado;
    ...
    public ResultSet existenciaModelo(String modelo){
        try {
            super.getStmt();
            resultado = stmt.executeQuery("SELECT  `producto`.`id`,`modelo`,`procesador`,`ram`,`disco`,`precio`,`nombre` FROM `producto` inner join `categoria` on `producto`.`categoria_id`=`categoria`.`id` where modelo like '%"+modelo+"%'");
        } catch (SQLException ex) {
            Logger.getLogger(AccesoProducto.class.getName()).log(Level.SEVERE, null, ex);
        }
        return resultado;
    }
     public ResultSet existenciaCategoria(int categoria){
        try {
            super.getStmt();
            resultado = stmt.executeQuery("SELECT  `producto`.`id`,`modelo`,`procesador`,`ram`,`disco`,`precio`,`nombre` FROM `producto` inner join `categoria` on `producto`.`categoria_id`=`categoria`.`id` where categoria_id="+categoria);
        } catch (SQLException ex) {
            Logger.getLogger(AccesoProducto.class.getName()).log(Level.SEVERE, null, ex);
        }
        return resultado;
